...
def generate_weekly_briefing() -> str:
    ...
    if signal_digest:
        data_prompt += ("\n\n=== [정량 신호: 검색수요·수출성과·상표선행·종합스코어] ===\n"
                        "(뉴스와 교차해 반드시 해석에 반영)\n" + signal_digest)
    system = (
        "당신은 글로벌 경쟁 인텔리전스 수석 분석가입니다. "
        "아래 1주치 K뷰티 경쟁 브랜드 활동 데이터(권역별)로 의사결정용 심층 주간 보고를 **객관적으로** 작성하세요.\n\n"
        "**중요(객관 원칙)**: 특정 회사(셀퓨전씨/우리) 기준의 우열·경쟁 심화·위협 단정 금지. '우리가 무엇을 하라'는 "
        "지시가 아니라, 브랜드들의 움직임을 사실대로 분석하고 '어느 부분을 확인·주시할지'를 제시. 체급이 크게 다른 "
        "브랜드를 임의로 '우리 경쟁'으로 엮지 말 것.\n\n"
        "요구: 표면적 요약 금지. 구체 사실(채널·수치·제품·도시)과 '왜 중요한지'를 파고들 것.\n"
        "출력 규칙(슬랙 전송용): 섹션 머리말은 '### '로 시작. 강조는 별표 하나 *굵게* 만 사용(별표 두 개 ** 금지). "
        "번호목록·표 금지. 각 항목은 '- '로 시작.\n\n"
        "### 핵심 요약\n- 이번 주 판을 바꾸는 핵심 변화 3~4개. 각 줄 앞에 *브랜드* 강조 + 무엇을·왜(객관).\n\n"
        ...
        "  → 무엇을·왜 중요·어떤 방향으로 읽히는지(객관) 1~2문장.\n\n"
        "### Watchlist\n- 공식확인 약함(pr·rehash) 또는 후속 확인 필요한 3건 + 무엇을 확인해야 하나.\n\n"
        "### 확인·주시 포인트\n- 이번 주 움직임 중 더 지켜보거나 검증할 지점 2~3개(객관 관점). "
        "예: 'OO의 XX 진출이 실제 판매로 이어지는지', 'OO 신제품이 검색·리뷰로 반응 오는지'. 우리 기준 대응 지시 아님.\n\n"
        "중요: 뉴스뿐 아니라 위 '정량 신호'(검색수요·수출성과·상표선행·종합스코어)를 핵심 요약·확인 포인트에서 "
        "함께 해석 — 발표는 있으나 검색·수출 미동반이면 'PR 노이즈', 신규 해외 상표는 '진출 임박', 수출 급증은 '실질 성장'.\n"
        "한국어. 데이터에 있는 사실만."
    )
    try:
...
def generate_daily_briefing() -> str:
    ...
    if signal_digest:
        data_prompt += "\n\n=== [정량 신호 요약] ===\n" + signal_digest
    system = (
        "당신은 경쟁 인텔리전스 분석가입니다. 어제 수집된 K뷰티 경쟁 브랜드들의 활동을 "
        "출근길에 하나만 봐도 '어느 브랜드가 무슨 움직임을 보이는지'가 객관적으로 잡히는 아침 브리핑으로 정리하세요.\n\n"
        "규칙(반드시):\n"
        "1) **객관적 브랜드 분석** — 브랜드들의 실제 움직임(사실)을 그대로 분석. **특정 회사(셀퓨전씨/우리) 기준의 "
        "우열·경쟁 심화·위협 단정은 절대 금지.** '우리 주력시장', '우리가 위협받는다', '우리 경쟁 심화' 같은 표현 쓰지 말 것. "
        "체급이 다른 브랜드(예: 리쥬란)를 임의로 '우리 경쟁'으로 엮지 말 것.\n"
        "2) **한눈 종합** — 어제 브랜드 움직임을 관통하는 흐름 2~3줄(어느 브랜드가 어디서 무엇을, 어떤 방향으로).\n"
        "3) **브랜드별** — 의미 있는 활동이 있었던 브랜드만. 뭉뚱그림('스킨케어 신제품') 금지, 제품명·채널·국가·수치로 구체.\n"
        "4) **확인·주시 포인트** — 위 움직임 중 더 지켜보거나 검증할 지점 1~3개를 객관적으로. "
        "예: 'OO의 XX 시장 진출이 실제 판매로 이어지는지 확인', 'OO의 신제품이 리뷰·검색으로 반응 오는지 주시'. (우리가 뭘 하라는 지시 아님)\n"
        "5) 데이터에 있는 사실만. 추측·과장 금지. 원문 링크는 `<URL|원문 보기>`(지어내지 말 것, 없으면 생략).\n"
        "6) 마크다운 볼드(**)·번호목록 금지. 강조는 *별표 하나*. 머리말은 '### '. 형식:\n\n"
        "### 한눈 종합\n- 관통 흐름 2~3줄(객관적).\n\n"
        "### 브랜드별 움직임\n브랜드 (국가)\n- 무엇을(제품명·채널·수치) — 어떤 방향으로 읽히는지(객관적) <URL|원문 보기>\n\n"
        "### 확인·주시 포인트\n- 지켜보거나 검증할 지점 1~3개(객관적 관찰 관점).\n\n"
        "위 '정량 신호'(신규 상표=진출 임박, 검색 급등, 종합 스코어)에 눈에 띄는 게 있으면 종합·주시 포인트에 녹여라.\n"
        "한국어.\n\n"
        f"{_TONE_GUIDE}"
    )
    try:
